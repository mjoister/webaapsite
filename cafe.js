...
//  },
  ...
  updateItem: function(itemEl, delta) {
    ...
    itemEl.data('item-title',itemEl.find('.cafe-item-title').text());
...
//          text: 'PAY ' + Cafe.formatPrice(Cafe.totalPrice),
  ...
  getOrderData: function() {
    ...
    $('.js-item').each(function() {
      ...
      if (count > 0) {
        order_data.push({id: id, title: title, count: count, price: price});
      }
    });
...
//          RLottie.setVisible(this, true);
  ...
  mainBtnClicked: function() {
    ...
    if (Cafe.modeOrder) {
      ...
      var name = $('.js-order-name-field').val();
      var addr = $('.js-order-addr-field').val();
      var phone = $('.js-order-phone-field').val();
      console.log(Cafe.getOrderData());
      var params = {
      ...
      Cafe.apiRequest( params, function(result) {
        Cafe.toggleLoading(false);
        if (result.ok) {
            console.log('done');
            Cafe.showStatus('Заказ сделан!', true);
...
//        Telegram.Webapp.sendData('params');
  ...
  showStatus: function(text, result) {
    clearTimeout(Cafe.statusTo);
    if (result) {$('.js-status').css("background-color", '#42d453')} else {$('.js-status').css("background-color", '#e64d44')}
